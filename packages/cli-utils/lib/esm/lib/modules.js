...
const resolve = semver.satisfies(process.version, '>=10.0.0')
    ? require.resolve
    : resolveFallback;
export const resolveModule = function (request, context) {
...
export const loadModule = function (request, context, force = false) {
    // createRequire doesn't work with jest mocked fs
    // (which we used in tests for cli-service)
    if (process.env.MAF_CLI_TEST && context === '/') {
...
export const clearModule = function (request, context) {
    const resolvedPath = resolveModule(request, context);
    if (resolvedPath) {
        clearRequireCache(resolvedPath);
    }
